    ...
    await fs.readdirSync(`./dist/${path}`).forEach(
        async (file) =>
            await import(`../${path}/${file}`).then(async (command) => {
                ...
                client.logger.info(`Command ${_command.name} (${file}) loaded. (./${path})`)
...
export default async (options: Required<Omit<Types.IBotOptions, 'clientOptions' | 'handleInteraction'>>) => {
    if (!options.config.token || !options.config.clientId || !options.config.ownerId)
    ...
    await registCommand(options.path?.path!).then(async (commands) => {
        const rest = async (route: RouteLike, _options: RequestData = { body: commands }) =>
        ...
        if (options.config.devGuild) {
            ...
            await rest(Routes.applicationCommands(options.config.clientId ?? ''))

            options.logger.info(`Registered ${commands.length} commands. (GLOBAL)`)
        }
    })
}
